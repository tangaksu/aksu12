...
function aksu_injection_defend() {
    ...
    // SQL注入高风险特征（结合互联网主流WAF、护卫神、OWASP等安全规则）
    $sql_rules = [
        // 基础符号和注释
        '/(?:\%27)|(?:\')|(?:\-\-)|(?:\%23)|(#)/i',
        '/\/\*.*\*\//s',
        // 常见SQL关键词和函数
        '/\b(select|update|delete|insert|drop|create|alter|truncate|replace|handler|load|grant|revoke|union|into|outfile|dumpfile|information_schema|sleep|benchmark|char|declare|cast|convert|extractvalue|updatexml|group_concat|xp_cmdshell|waitfor|hex|unhex)\b\s*[\(\s]/i',
        // 逻辑绕过
    ...
    // XSS注入高风险特征（参考OWASP、护卫神等主流正则及社区案例）
    $xss_rules = [
    ...
    foreach ($sql_rules as $rule) {
        if (preg_match($rule, $all)) {
            ...
            aksu_defense_die('Forbidden SQL Injection', 403);
        }
    }
    ...
    foreach ($xss_rules as $rule) {
        if (preg_match($rule, $all)) {
            ...
            aksu_defense_die('Forbidden XSS Injection', 403);
        }
    }
}
