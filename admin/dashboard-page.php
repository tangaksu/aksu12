...
function aksu_dashboard_page() {
    ...
    $high_freq_ips = $wpdb->get_results($wpdb->prepare(
        "SELECT ip, COUNT(*) as cnt FROM $table WHERE time > %s AND type IN ('cc','injection','useragent','useragent_blacklist','scan','cookie','upload','uri') AND ip <> '' GROUP BY ip ORDER BY cnt DESC LIMIT 7",
        $since
    ));
